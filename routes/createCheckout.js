...
router.all('/create-checkout', async (req, res) => {
  try {
    ...
    // Сохраняем по deal_number в checkoutStatus
    try {
      const checkoutStatus = require('./checkoutStatus');
      checkoutStatus.saveCheckout(params.deal_number, {
        amount: dealCostUSD,
        offerTitle: params.offer_title || 'Order',
      ...
      console.error('[CREATE-CHECKOUT] Storage error (non-critical):', storageError.message);
